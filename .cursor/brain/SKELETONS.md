...
- YouTube subtitle fetch: using `all` languages in `--sub-langs` can trigger HTTP 429. The default is a short list tried **sequentially**; widen with `YT_TRANSCRIPT_SUB_LANGS` only when needed. One automatic retry per language after 429 uses `YT_TRANSCRIPT_SUB_429_RETRY_MS` (default 3500 ms).
- Whisper shell execution: `runWhisperToVtt()` still uses `sh -c`, but `{{audio}}` / `{{outdir}}` are substituted via POSIX single-quote wrapping (`quoteForPosixShSingle`). Do not wrap those placeholders in extra `\"...\"` in the template (including the default `YT_TRANSCRIPT_WHISPER_CMD`); nested quotes pass literal `'` into argv. Changing substitution or adding placeholders without the same treatment reintroduces injection risk.
- Summary command shell: `runSummaryShellCommand()` uses `sh -c` with the same quoting for all `{{…}}` values from `buildCommandVars` (paths + `VIDEO_ID`). User templates must not wrap placeholders in extra `"..."`; each placeholder expands to one safe shell word.
- URL scope vs docs: default CLIs enforce a YouTube hostname allowlist (`assertYoutubeWatchUrl`); non-YouTube URLs require `YT_TRANSCRIPT_ALLOW_ANY_URL=1` or `--allow-any-url`. Video ids from yt-dlp are still validated with `assertSafeVideoIdForPath` at `parseVideoInfoFromDumpJson`.
- Summary validator path: optional `--artifacts-root` on `summaryValidatorCli` limits reads when the CLI is scripted; default behavior stays path-as-given for local dev.
