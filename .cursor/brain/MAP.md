...
- **Local prepare:** `agentWorkflowCli` runs `prepareAgentWorkflow()` (single URL, or **`--batch-file`** / stdin for sequential prepares) → `runPipeline()` (manual or auto VTT, Whisper fallback) → optional **`verification-hints.md`** (URLs + transcript anchors; no network) and optional **`keyframes/`** JPEGs (`downloadMergedVideo` + `ffmpeg` when `YT_TRANSCRIPT_KEY_FRAMES=1` or `--key-frames`) → `summary-prompt.md` includes **`{{SUPPLEMENTARY_CONTEXT}}`** when those artifacts exist → artifact bundle. **`agent:complete`** (`agentCompleteCli`) can chain the same prepare, a user **`YT_SUMMARY_CMD`** shell, and `validateSummary`. On validation failure, CLIs emit JSON on stdout and human-oriented hints on stderr (`summaryValidationHints.ts`); **`agent:check-summary`** follows the same stdout/stderr split.
...
- Condensed **open backlog** (goal: shrink to zero and delete): `docs/technical-debt-roadmap.md` (includes **Current focus**: denser handoff, fact-fetch, optional ffmpeg timecoded frames; batch deferred).
...
Each video folder under `artifacts/videos/<slug>-<id6>/` holds `transcript.md`, `summary-prompt.md`, `summary.<replyLanguage>.md`, `manifest.json`, and `cursor-handoff.md` (guided Cursor checklist from `buildCursorHandoffMarkdown()` in `agentWorkflow.ts`). Directory name is built by `buildArtifactDirName(videoTitle, videoId)` in `src/shared/safePathSegment.ts`: slugified title (ASCII, lowercased, NFKD-stripped, basic Cyrillic transliterated, non-alnum → `-`, ≤ 50 chars at word boundary) joined with the first 6 chars of `videoId`. Falls back to the raw `videoId` when the title slug is empty (emoji-only or non-ASCII titles). `manifest.json` includes `transcriptFileChars`, `transcriptBodyChars`, **`videoDescription`** (full page text from YouTube), **`videoDescriptionAlignment`**, **`videoDescriptionAlignmentPolicy`**, **`videoDescriptionLexicalOverlap`**, **`videoDescriptionTokenCount`**, **`videoDescriptionOmittedFromTranscriptYaml`**. `transcript.md` YAML includes **`description`** only when alignment is not `low` (misaligned promos are dropped from the file but kept in the manifest). Agent order: read manifest and prompt, write the language-specific summary file, validate, use `transcript.md` only as fallback or deep check.
