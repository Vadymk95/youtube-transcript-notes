...
**How to use (walking order):** Work **from the top down** (easiest → hardest) unless a lower item blocks you. For each bullet: implement the smallest slice → run **`npm run ci`** → if behavior or trust model changed, sync **`.cursor/brain/`** (`MAP.md`, `DECISIONS.md`, `SKELETONS.md` as needed) per repo rules. **Exploratory** items (long nested sections) need a **spec or agent playbook** before touching core code; they must keep the **canonical** path **URL → transcript → summary → `agent:check-summary`** unless the item explicitly extends the contract. When something ships, **remove** its bullet here and add a one-line note under **Recently shipped** (do not duplicate full specs in both places).

## Backlog (easiest → hardest to implement)

1. **Expand transcript quality fixtures** (`fixtures/transcript-quality/`). _(Mostly new fixture files + occasional loader/harness tweaks.)_

2. **External benchmark** (fixed video set vs raw YouTube / `yt-dlp` alone / npm transcript helpers); document regression expectations. _(Harness/docs discipline; maintaining a golden set.)_

3. **Bundled or guided summarizer** for users who will not write `YT_SUMMARY_CMD` (stay local-first; no remote API as default). _(Product/UX; may wrap or document a default local path.)_

4. **Optional summary shapes** beyond the strict handoff contract (e.g. short summary, outline) — requires aligned prompt + validator changes. _(Cross-cutting: `prompts/video-notes-prompt.md`, `outputLanguage.ts`, `summaryContract.ts`, tests.)_

5. **Batch URLs / playlists** (power-user; watch scraping risk). _(CLI orchestration + policy.)_

6. **Optional multimodal / key-frame context (exploratory)** — **full-picture** summaries (speech + on-screen UI/slides) without making the core CLI depend on any single LLM vendor; **not started** (weigh pros/cons before coding). **Rationale:** depends on Cursor agent UX, optional contract extensions, and video-on-disk workflow — **not** part of the core transcript/summary moat until the text path is boringly solid.
...
7. **Searchable archive / semantic retrieval workflow.** _(New workflow layer; indexing, storage, retrieval policy.)_

8. **UI or browser extension.** _(Large product surface; out of band for CLI moat.)_

9. **Optional claim verification / fact-fetch pass (exploratory)** — treat `summary.<replyLanguage>.md` as **structured handoff**, not **ground truth** for factual news; **not started** (spec + recipe before code).
    ...
    - **Critique of this backlog item itself (so we do not overfit the plan):**
        ...
        - Full automation of “truth” is **not** a realistic outcome here; the valuable increment is **transparent, optional** human-in-the-loop or agent-assisted **evidence pointers**, not a second truth oracle.
