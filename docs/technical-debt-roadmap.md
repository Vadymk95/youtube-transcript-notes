...
1. **External benchmark** (fixed video set vs raw YouTube / `yt-dlp` alone / npm transcript helpers); document regression expectations. _(Harness/docs discipline; maintaining a golden set.)_

2. **Bundled or guided summarizer** for users who will not write `YT_SUMMARY_CMD` (stay local-first; no remote API as default). _(Product/UX; may wrap or document a default local path.)_

3. **Optional summary shapes** beyond the strict handoff contract (e.g. short summary, outline) — requires aligned prompt + validator changes. _(Cross-cutting: `prompts/video-notes-prompt.md`, `outputLanguage.ts`, `summaryContract.ts`, tests.)_

4. **Batch URLs / playlists** (power-user; watch scraping risk). _(CLI orchestration + policy.)_

5. **Optional multimodal / key-frame context (exploratory)** — **full-picture** summaries (speech + on-screen UI/slides) without making the core CLI depend on any single LLM vendor; **not started** (weigh pros/cons before coding). **Rationale:** depends on Cursor agent UX, optional contract extensions, and video-on-disk workflow — **not** part of the core transcript/summary moat until the text path is boringly solid.
...
6. **Searchable archive / semantic retrieval workflow.** _(New workflow layer; indexing, storage, retrieval policy.)_

7. **UI or browser extension.** _(Large product surface; out of band for CLI moat.)_

8. **Optional claim verification / fact-fetch pass (exploratory)** — treat `summary.<replyLanguage>.md` as **structured handoff**, not **ground truth** for factual news; **not started** (spec + recipe before code).
